...
export function toModulePath(options: ToModulePathOptions): string {
  if (parseExternalModuleName(options.from)) return options.from;
  ...
  if (from.startsWith("../")) return normalize(from);
  const result = `${parentDir}/${from}`;
  if (result.startsWith("./")) return `./${normalize(result)}`;
...
export async function getExternalModulePath(
  parsed: ParsedExternalModuleName,
  sourceDir: string,
): Promise<string> {
  ...
  if (!mainPackageDir) throw new Error(`Cannot find the package directory of '${sourceDir}'`);

  const depPackageDir = join(mainPackageDir, "node_modules", parsed.packageName);

  if (parsed.additionalPath) return join(depPackageDir, parsed.additionalPath);
  ...
  if (additionalPath) return join(depPackageDir, removeModuleNameExtension(additionalPath));
...
async function findParentPackageDir(dir: string): Promise<string | undefined> {
  ...
  while (true) {
    if (await pathExists(join(resolvedDir, "package.json"))) return resolvedDir;
    if (resolvedDir === "/" || resolvedDir === ".") return;
