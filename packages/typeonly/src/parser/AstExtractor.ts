...
export default class AstExtractor extends TypeOnlyParserListener {
  ...
  enterDeclarations(_ctx: AntlrRuleContext) {
    this.childTypes = new Map();
    this.compositeMap = new Map();
    this.ast = {};
  }

  exitDeclarations(_ctx: AntlrRuleContext) {
  ...
  enterClassicImport(ctx: AntlrRuleContext) {
    ...
    if (ctx.namedImportContent().namedMember()) {
      ...
      namedMembers.forEach((member: any, index: number) => {
        ...
        if (member.IDENTIFIER()[1]) mb.as = member.IDENTIFIER()[1].getText();
        members[index] = mb;
      });
    ...
    if (!this.ast.declarations) this.ast.declarations = [];
  ...
  enterNamedInterface(ctx: AntlrRuleContext) {
    ...
    if (ctx.EXPORT()) this.currentNamedInterface.exported = true;
    if (ctx.interfaceExtends()) {
      const names = Object.values(ctx.interfaceExtends().typeName()).map((child: any) =>
        child.getText(),
      );
  ...
  exitNamedInterface(ctx: AntlrRuleContext) {
    assertExists(this.currentNamedInterface);
    ...
    if (this.interfaceStack.length > 0) throw new Error("InterfaceStack should be empty");
    this.checkMissingChildren();
  }
  ...
  exitAnonymousInterface(ctx: AntlrRuleContext) {
    ...
    if (!interf) throw new Error("InterfaceStack should not be empty");
    ...
    if (interf.entries) {
      ...
      for (const entry of interf.entries) {
        ...
        if (entry.whichEntry === "indexSignature") ++indexSignatureNb;
      ...
      if (mappedIndexSignatureNb > 0) {
        ...
        if (mappedIndexSignatureNb > 1 || (mappedIndexSignatureNb === 1 && otherPropertyNb > 0)) {
          throw new Error(
            "Synthax Error : An Interface must have one property which is a mappedIndexSignature property",
          );
        }
      }
  ...
  enterNamedType(ctx: AntlrRuleContext) {
    ...
    if (ctx.EXPORT()) namedType.exported = true;
  ...
  exitNamedType(ctx: AntlrRuleContext) {
    assertExists(this.currentNamedType);
    this.addGrabbedCommentsResultTo(this.comments.grabCommentsOf(ctx), {
  ...
  enterProperty(ctx: AntlrRuleContext) {
    if (this.interfaceStack.length === 0) throw new Error("Missing interfaceStack");
  ...
  enterMappedIndexSignature(ctx: AntlrRuleContext) {
    ...
    this.setAstChildRegistration((type) => {
      mappedIndexSignature.keyInType = type;
    }, ctx.aType()[0]);
    this.setAstChildRegistration((type) => {
      mappedIndexSignature.type = type;
    }, ctx.aType()[1]);
  }
  ...
  enterAType(ctx: AntlrRuleContext) {
    ...
    } else if (isThere(ctx.UNION()) || isThere(ctx.INTERSECTION())) {
  ...
  exitAType(ctx: AntlrRuleContext) {
    if (isThere(ctx.UNION()) || isThere(ctx.INTERSECTION())) {
  ...
  processCompositeType(ctx: AntlrRuleContext) {
    const compositeType: AstCompositeType = {
      whichType: "composite",
      op: isThere(ctx.INTERSECTION()) ? "intersection" : "union",
      types: [],
    };
  ...
  processEndOfCompositeType(ctx: AntlrRuleContext) {
    ...
    const mergeRight =
      typeof right !== "string" && right.whichType === "composite" && right.op === compositeType.op;
    if (mergeLeft || mergeRight) {
      const types: AstType[] = [];
      if (mergeLeft) types.push(...(left as AstCompositeType).types);
      else types.push(left);
      if (mergeRight) types.push(...(right as AstCompositeType).types);
      else types.push(right);
  ...
  processFunctionType(ctx: AntlrRuleContext) {
    ...
    functionParameters.forEach((param: AntlrRuleContext, index: number) => {
      this.setAstChildRegistration((astType) => {
        if (!functionType.parameters) functionType.parameters = [];
  ...
  enterFunctionProperty(ctx: AntlrRuleContext) {
    ...
    functionParameters.forEach((param: AntlrRuleContext, index: number) => {
      this.setAstChildRegistration((astType) => {
        if (!functionProperty.parameters) functionProperty.parameters = [];
  ...
  registerAstChild(astType: Partial<AstType>, aType: AntlrRuleContext) {
    ...
    if (!cb) throw new Error(`Unexpected child type: ${aType.getText()}`);
  ...
  setAstChildRegistration(cb: SetType, aType: AntlrRuleContext) {
    ...
    if (aType.typeName() || aType.signatureType()) this.registerAstChild(aType.getText(), aType);
  }

  checkMissingChildren() {
    if (this.childTypes.size > 0) throw new Error(`Missing children: ${this.childTypes.size}`);
    if (this.compositeMap.size > 0)
      throw new Error(`Remaining composite: ${this.compositeMap.size}`);
  }

  addGrabbedCommentsResultTo(
    result: GrabbedCommentsResult,
    options: {
    ...
    if (result.standaloneCommentsBefore)
      this.addStandaloneCommentsTo(
        result.standaloneCommentsBefore,
        standaloneBeforeTo,
        parentInterface,
      );
    if (result.docComment) annotate.docComment = result.docComment;
    if (result.inlineComments.length > 0) annotate.inlineComments = result.inlineComments;
  }

  addStandaloneCommentsTo(
    grabbedComments: GrabbedComment[],
    to?: "interface",
    parentInterface?: AstInterface,
  ) {
    if (grabbedComments.length === 0) return;
    if (!to) {
      if (!this.ast.declarations) this.ast.declarations = [];
      ...
      if (!parentInterface.entries) parentInterface.entries = [];
      parentInterface.entries.push(
        ...grabbedComments.map(
          ({ text, syntax }) =>
            ({
              whichEntry: "comment",
    ...
    } else throw new Error(`Invalid 'to' option: ${to}`);
  }
}
