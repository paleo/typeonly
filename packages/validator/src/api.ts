...
export type CreateValidatorOptions =
  ...
  | AsyncReadModulesValidatorOptions;

export function createValidator(options: SyncReadModulesValidatorOptions): TypeOnlyValidator;
export function createValidator(
  options: AsyncReadModulesValidatorOptions,
): Promise<TypeOnlyValidator>;
export function createValidator(options: CreateValidatorOptions): any {
  if (isSyncReadModulesOptions(options)) return createValidatorSync(options);
...
export function createValidatorFromModules(
  modules: Modules,
...
function getDefaultModuleName(modules: Modules, typeName: string): string {
  ...
  for (const moduleName of Object.keys(modules)) {
    ...
    if (module.namedTypes[typeName]) candidates.push(moduleName);
  }
  if (candidates.length === 0) throw new Error(`Cannot find type '${typeName}' in modules.`);
  if (candidates.length > 1)
    throw new Error(
      `There are several module candidates for type '${typeName}': '${candidates.join("', '")}'.`,
    );
  return candidates[0];
}
