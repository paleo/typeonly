...
import type { ValidatorOptions } from "./api.js";
import { makeErrorMessage, typeAsString } from "./error-message.js";
import { hasAncestor } from "./helpers.js";
...
export default class Validator {
  ...
  private checkType(
    type: Type,
    val: unknown,
    parentContextMessage?: () => string,
    scope?: Scope,
  ): InternalResult {
    const validator = this.typeValidators[type.kind];
    if (!validator) throw new Error(`Unexpected kind: ${type.kind}`);
    ...
    if (!result.valid && parentContextMessage) {
      ...
      if (last) last.parentContextMsg = parentContextMessage();
    }
    return result;
  }
  ...
  private checkTypeName(type: TypeName, val: unknown): InternalResult {
    if (type.group === "primitive") {
      ...
      } else if (typeof val === type.refName) return { valid: true };
    ...
    if (type.group === "global") {
      if (hasAncestor(val, type.refName)) return { valid: true };
  ...
  private checkInterface(type: Interface, val: unknown, scope: Scope | undefined): InternalResult {
    if (!val || typeof val !== "object") return { valid: false, unmatchs: [{ type, val }] };

    const context = this.makeInterfaceContext(type, val as InterfaceObject, scope);

    if (type.mappedIndexSignature) throw new Error("MappedIndexSignature not yet implemented.");

    if (type.indexSignature) this.checkIndexSignature(type.indexSignature, context);

    if (type.properties) this.checkProperties(type.properties, context);

    if (context.scopeOwner) this.endOfInterfaceScopeOwner(context);

    return (context.scopeOwner && context.interfaceScope.firstInvalid) || { valid: true };
  }

  private makeInterfaceContext(
    type: Interface | CompositeType,
    val: InterfaceObject,
    ...
    if (resultScope?.interfaceScope) {
      interfaceScope = resultScope.interfaceScope;
      scopeOwner = false;
    } else {
      interfaceScope = {
      ...
      resultScope = { interfaceScope };
      scopeOwner = true;
    }
    return { type, val, scope: resultScope, interfaceScope, scopeOwner };
  }
  ...
  private endOfInterfaceScopeOwner(context: InterfaceContext) {
    ...
    if (
      !this.options.acceptAdditionalProperties &&
      !scope.firstInvalid &&
      scope.remainingPropNames.size > 0
    ) {
  ...
  private checkIndexSignature(indexSignature: IndexSignature, context: InterfaceContext) {
    ...
    for (const [propName, childVal] of Object.entries(val)) {
      let valid = true;
      if (indexSignature.keyType === "number" && Number.isNaN(Number(propName))) {
        if (!interfaceScope.firstInvalid) {
          const cause = `Property name ${propName} is not a number`;
          interfaceScope.firstInvalid = {
            valid: false,
            unmatchs: [{ type: indexSignature.of, val, cause }],
          };
        }
        valid = false;
      }
      const result = this.checkType(
        indexSignature.type,
        childVal,
        () => `property '${propName}' (from index signature)`,
      );
      if (!result.valid) {
        if (!interfaceScope.firstInvalid) {
  ...
  private checkProperties(properties: Properties, context: InterfaceContext) {
    ...
    for (const property of Object.values(properties)) {
      ...
      if (result.valid) {
        ...
        if (!interfaceScope.firstInvalid) interfaceScope.firstInvalid = result;
        ++interfaceScope.invalidPropCount;
        if (property.type.kind === "literal") {
          ++interfaceScope.invalidLiteralCount;
        }
  ...
  private checkLiteralType(type: LiteralType, val: unknown): InternalResult {
    if (type.literal !== val) return { valid: false, unmatchs: [{ type, val }] };
    return { valid: true };
  }
  ...
  private checkCompositeType(
    type: CompositeType,
    val: unknown,
    scope: Scope | undefined,
  ): InternalResult {
    if (type.op === "union") return this.checkCompositeUnion(type, val);
  ...
  private checkCompositeIntersection(
    type: CompositeType,
    val: unknown,
    scope: Scope | undefined,
  ): InternalResult {
  ...
  private checkKeyofTypeWith(type: Type, val: unknown): InternalResult {
    if (type.kind === "interface") {
      if (type.indexSignature) {
        if (type.indexSignature.keyType === typeof val) return { valid: true };
      }
      ...
      for (const propertyName of Object.keys(type.properties || {})) {
        if (propertyName === val) return { valid: true };
      }
      return { valid: false, unmatchs: [{ type, val }] };
    }
    ...
    if (type.kind === "composite") {
      if (type.op !== "intersection") throw new Error("Cannot use keyof on a union type.");
      for (const itemType of type.types) {
        if (this.checkKeyofTypeWith(itemType, val).valid) return { valid: true };
      }
      return { valid: false, unmatchs: [{ type, val }] };
    }
  ...
  private checkImportedTypeRef(
    type: ImportedTypeRef,
    val: unknown,
    scope: Scope | undefined,
  ): InternalResult {
  ...
  private checkMemberTypeWith(
    ...
    if (type.kind === "interface") {
      ...
      if (!memberType) throw new Error(`Missing member '${name}' in interface`);
    ...
    if (type.kind === "tuple") {
      ...
      if (!memberType) throw new Error(`Missing member '${name}' in tuple`);
    ...
    if (type.kind === "composite") {
      if (type.op !== "intersection") throw new Error("Cannot use member type on a union type.");
      for (const itemType of type.types) {
        if (this.checkMemberTypeWith(itemType, val, memberName).valid) return { valid: true };
      }
      return { valid: false, unmatchs: [{ type, val }] };
    }
  ...
  private checkFunctionType(type: FunctionType, val: unknown): InternalResult {
    if (typeof val === "function") return { valid: true };
